    ...
    <div class="post-card">
        <div class="card white lighten-2 no-shadow" style="border-radius: 0">
            ...
            <div class="card-action">
                <div class="btn waves-effect waves-light red no-shadow white-text"
                     onclick="toggle_confirmation('none','/admin/vote/ticket/activate/')">全部禁用
                </div>
                <div class="btn waves-effect waves-light purple no-shadow white-text"
                     onclick="toggle_confirmation('all','/admin/vote/ticket/activate/')">全部激活
                </div>
                <div class="btn waves-effect waves-light green no-shadow white-text" onclick="location.href=''">刷新本页
                </div>
            </div>
        </div>
    </div>
    ...
    <form id="ticket-range-form" action="/admin/vote/ticket/toggle/with/range" method="post" accept-charset="utf-8">
        <div class="post-card">
            <div class="card white lighten-2 no-shadow" style="border-radius: 0">
                ...
                <div class="card-action">
                    <p>
                        <input class="with-gap" name="ticket-range" type="radio" id="ticket-range-activate"
                               onclick="setRangeStatus(this.id)"/>
                        <label for="ticket-range-activate">激活范围内的Ticket</label>
                    </p>
                    <p>
                        <input class="with-gap" name="ticket-range" type="radio" id="ticket-range-disable"
                               onclick="setRangeStatus(this.id)"/>
            ...
            <table class="centered striped responsive-table">
                ...
                @foreach($ticket as $ticket)
                    <tr>
                        ...
                        <td><?php foreach ($ticket->usedForVote() as $voteid) {
                                echo $voteid . ', ';
                            }?></td>
                        <td>
                            <button class="btn waves-effect waves-light pink no-shadow"
                                    onclick="toggle_confirmation({{$ticket->id}},'/admin/vote/clearAnswers/ticket/')">清空
                            </button>
                        </td>
                        <td>
                            <button class="btn waves-effect waves-light orange no-shadow"
                                    onclick="toggle_confirmation({{$ticket->id}}, '/admin/vote/ticket/toggle/')">{{($ticket->active) ? '禁用' : '启用'}}</button>
                        </td>
                        <td>
                            <button class="btn waves-effect waves-light orange no-shadow"
                                    onclick="print_ticket({{$ticket->id}},'{{$ticket->string}}')">
                                打印
                            </button>
                        </td>
                    </tr>
...
@section('script')
    <script type="text/javascript">
        ...
        function print_ticket(id, str) {
            htmlstr = "<h1 align='center'>VOICE of HFI 2017</h1>" +
                "<p align='center'>C.L.A.P. x HFIProgramming</p>" +
//                "<h4 align='center'>Voting Ticket</h4>" +
                "<div align='center'><img id=\"qrimg\" height=180 width=180 src=\"https://www.west.cn/web/tool/codepayimg?uuid=https://notice.hfi.me/vote/ticket/" + str + "\"></div>" +
		"<p align='center'><i>This ticket has id <strong>" + id + "</strong>.<br> Please try not to lose it.</i><br>Bilibili Live Room #: 2289749</p>";
            data = {html: htmlstr};
            $.ajax({
